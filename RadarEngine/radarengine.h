...
public:
    ...
    ~GZ() { qDebug()<<Q_FUNC_INFO<<m_log_name<<"destroyed"; }
    void ResetBogeys()
    {
        m_bogey_count = 0;
        m_running_count = 0;
...
class ADSBReceiver : public QThread
{
    Q_OBJECT

public :
    explicit ADSBReceiver(QObject *parent = 0,QString addr="", uint port=0);
    ~ADSBReceiver();
...
signals:
    void signal_call_sign(QByteArray addr, QString call_sign, quint8 EC);
    void signal_pos(QByteArray addr, quint8 ss, bool nic, quint32 alt,bool time,bool odd,
                    double lat, double lon);
    void signal_vel(QByteArray addr, quint8 st, bool ic, quint8 nac, bool vert_src,
...
private:
    ...
    bool isCRCValid(const unsigned char *data,int len);
    QByteArray getMODESICAO(const unsigned char *data,int len);
    void decode_ID(QByteArray addr, const unsigned char *data);
    void decode_Pos(QByteArray addr, const unsigned char *data);
    void decode_Velocity(QByteArray addr, const unsigned char *data);

};

struct ADSBObject
{
    double lat,lon,lat_odd,lat_even,lat_odd_cpr,lat_even_cpr;
    double lon_odd_cpr,lon_even_cpr,speed,course,air_speed,air_dir;
    quint32 vert_rate;
    double altitude;
    QByteArray icao;
    QString call_sign,country;
    quint64 time_tag,time_tag_epoch_odd,time_tag_epoch_even;
    quint8 emmitter_cat,ss,NL_odd,NL_even,ST,NAC;
    bool nic, time,IC,Vert_src,TAS;

};

class ADSBManager : public QObject
{
    Q_OBJECT

public:
    explicit ADSBManager(QObject *parent = 0,QString addr="", uint port=0);
    ~ADSBManager();

    QHash<QByteArray,ADSBObject>adsb_obj_map;
    ADSBReceiver *adsbReceive;

private slots:
    void handler_call_sign(QByteArray addr, QString call_sign, quint8 EC);
    void handler_pos(QByteArray addr, quint8 ss, bool nic, quint32 alt, bool time,
                     bool odd, double lat, double lon);
    void handler_vel(QByteArray addr, quint8 st, bool ic, quint8 nac, bool vert_src,
                    quint32 vert_rate, double vel, double crs);

private:

    ADSBObject createObject(QByteArray addr);
};
